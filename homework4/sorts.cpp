...
bool allPresent(const vector<Student>& students)
{
    ...
    for (const auto& s : students) present[s.id] = 1;
...
void sortUsingPtrs(vector<Student>& students, bool comp(const Student*, const Student*))
{
    ...
    vector<Student*> vecPtrs;
    for(int i = 0; i<students.size(); i++){
        vecPtrs.push_back(&auxStudents[i]);
    }
    sort(vecPtrs.begin(), vecPtrs.end(), compareStudentPtr);
    for(int i = 0; i < vecPtrs.size(); i++) {
        students[i] = *vecPtrs[i];
    }
